...
echo "🚀 Starting Keycloak deployment..."
...
if ! kubectl wait --for=condition=ready pod -l app=keycloak-postgres -n traefik --timeout=300s; then
    echo "❌ PostgreSQL failed to start. Check logs with: kubectl logs -n traefik -l app=keycloak-postgres"
    exit 1
fi
...
kubectl create rolebinding keycloak-operator-traefik-access \
  --clusterrole=edit \
...
if ! kubectl wait --for=condition=ready pod -l app.kubernetes.io/name=keycloak-operator -n keycloak --timeout=300s; then
    echo "❌ Keycloak operator failed to start. Check logs with: kubectl logs -n keycloak -l app.kubernetes.io/name=keycloak-operator"
    exit 1
fi

# Step 3: Deploy Database Secret (to keycloak namespace where Keycloak instance will run)
echo "🔐 Deploying database secret..."
kubectl create secret generic keycloak-db-secret -n keycloak --from-literal=username=postgres --from-literal=password=topsecretpassword --dry-run=client -o yaml | kubectl apply -f -
...
if ! kubectl wait --for=condition=ready pod -l app=keycloak -n keycloak --timeout=600s; then
    echo "❌ Keycloak failed to start. Check logs with: kubectl logs -n keycloak -l app=keycloak"
    echo "💡 Check if database secret exists: kubectl get secret keycloak-db-secret -n keycloak"
    exit 1
fi
...
SECRET_READY=false
for i in {1..30}; do
  if kubectl get secret keycloak-initial-admin -n keycloak >/dev/null 2>&1; then
    SECRET_READY=true
    break
  fi
  sleep 2
...
if [ "$SECRET_READY" = false ]; then
  ...
  echo "  kubectl get pods -n keycloak -l app=keycloak"
  echo "  kubectl get keycloak -n keycloak" 
  echo "  kubectl get keycloakrealmimport -n keycloak"
  echo ""
  echo "🔗 Access Keycloak:"
  echo "  kubectl port-forward svc/keycloak-service -n keycloak 9091:7000"
  exit 0
fi

# Get admin credentials
ADMIN_USER=$(kubectl get secret keycloak-initial-admin -n keycloak -o jsonpath='{.data.username}' | base64 -d 2>/dev/null)
ADMIN_PASS=$(kubectl get secret keycloak-initial-admin -n keycloak -o jsonpath='{.data.password}' | base64 -d 2>/dev/null)

# Start port forwarding in background
kubectl port-forward svc/keycloak-service -n keycloak 9091:7000 >/dev/null 2>&1 &
PF_PID=$!
...
echo "    kubectl get secret keycloak-initial-admin -n keycloak -o jsonpath='{.data.username}' | base64 -d && echo"
echo "    kubectl get secret keycloak-initial-admin -n keycloak -o jsonpath='{.data.password}' | base64 -d && echo"
